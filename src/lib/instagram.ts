...
export async function fetchPostComments(postId: string): Promise<InstagramComment[]> {
  ...
  try {
    ...
    const simulatedComments: InstagramComment[] = Array.from({ length: 15 }).map((_, i) => ({
...
export async function getPostData(postId: string): Promise<InstagramPost | null> {
  try {
