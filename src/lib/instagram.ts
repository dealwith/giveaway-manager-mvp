...
if (!INSTAGRAM_BUSINESS_ACCOUNT_ID) {
  console.warn('Instagram business account ID not configured. Some API calls may fail.');
}
...
async function findMediaIdByPermalink(permalink: string): Promise<string | null> {
  try {
    ...
    const response = await axios.get(`${INSTAGRAM_API.BASE_URL}/${INSTAGRAM_BUSINESS_ACCOUNT_ID}/media`, {
      params: {
        fields: 'id,permalink',
    ...
    const targetPost = posts.find((post: { permalink: string }) =>
      post.permalink === permalink || post.permalink.includes(extractPostShortcode(permalink) || '')
    );
    ...
    while (nextPage) {
      ...
      const foundPost = nextPosts.find((post: { permalink: string }) =>
        post.permalink === permalink || post.permalink.includes(extractPostShortcode(permalink) || '')
      );

      if (foundPost) {
        console.log(`Found media ID ${foundPost.id} for permalink ${permalink}`);
        return foundPost.id;
      }
...
export async function fetchPostComments(postUrlOrMediaId: string): Promise<InstagramComment[]> {
  try {
    if (!INSTAGRAM_ACCESS_TOKEN) {
    ...
    const response = await axios.get(`${INSTAGRAM_API.BASE_URL}/${mediaId}/comments`, {
      params: {
    ...
    const comments: InstagramComment[] = response.data.data?.map((comment: {
      id: string;
      username: string;
      text: string;
      timestamp: string;
    ...
    if (axios.isAxiosError(error)) {
      ...
      if (error.response?.status === 400) {
        ...
        if (errorCode === 100 && errorType === 'GraphMethodException') {
          throw new Error(`Invalid media ID. ${errorMessage || 'Please ensure you\'re using a valid Instagram media ID.'}`);
        }

        if (errorCode === 190) {
          throw new Error('Invalid Instagram access token. Please check your Instagram API credentials.');
        }

        if (errorMessage?.includes('does not exist')) {
          throw new Error('Media not found. The post may have been deleted or you don\'t have permission to access it.');
        }
      }

      if (error.response?.status === 404) {
        throw new Error('Instagram post not found. Please check the post URL and ensure you have access to it.');
      }
    }
...
export function findCommentsWithKeyword(comments: InstagramComment[], keyword: string): InstagramComment[] {
  const normalizedKeyword = keyword.toLowerCase().trim();

  console.log(`Searching for keyword "${keyword}" (normalized: "${normalizedKeyword}") in ${comments.length} comments`);
...
export async function sendDirectMessage(username: string, message: string): Promise<boolean> {
  try {
    ...
    console.warn('Instagram API does not support automated messaging. Manual process required.');
...
export async function getPostData(postUrlOrMediaId: string): Promise<InstagramPost | null> {
  try {
    let mediaId: string;
...
export async function processGiveaway(
  ...
  if (!isMediaId(postUrl) && !isValidInstagramPostUrl(postUrl)) {
    throw new Error('Invalid Instagram post URL or media ID. Please ensure the URL is in the format: https://instagram.com/p/SHORTCODE/ or provide a valid media ID.');
  }
  ...
  for (const comment of matchingComments) {
    const result = await processWinner(comment.username, comment.id, documentUrl);
    winners.push({
      username: comment.username,
