  ...
  const onSubmit = async (data: GiveawayFormValues) => {
    ...
    try {
      if (!giveaway) {
        ...
        if (reachedLimit) {
          setError(
            `You've reached your limit of ${PLANS[session.user.plan || SubscriptionPlan.FREE].giveawayLimit} giveaways. Please upgrade your plan to create more.`
          );
          setIsLoading(false);
          return;
        }
