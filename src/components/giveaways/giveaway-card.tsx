      ...
      <CardContent className="flex-1">
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center text-sm">
            ...
            <span>{format(new Date(giveaway.startTime), 'MMM d, yyyy h:mm a')}</span>
          </div>

          <div className="flex items-center text-sm">
            <span className="font-medium mr-2">End:</span>
            <span>{format(new Date(giveaway.endTime), 'MMM d, yyyy h:mm a')}</span>
          </div>

          {isActive && (
            <div className="flex items-center text-sm mt-2">
              <span className="font-medium mr-2">Active for:</span>
              <span>{formatDistanceToNow(new Date(giveaway.endTime), { addSuffix: true })}</span>
            </div>
          )}
