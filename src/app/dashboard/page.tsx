...
export default async function DashboardPage() {
  ...
  const activeGiveaways = giveaways.filter(giveaway => giveaway.status === GiveawayStatus.ACTIVE);
  const completedGiveaways = giveaways.filter(giveaway => giveaway.status === GiveawayStatus.COMPLETED);
            ...
            {giveaways.slice(0, 3).map((giveaway) => (
              <Card key={giveaway.id}>
                <CardHeader>
                  ...
                  <CardDescription>
                    {giveaway.status === GiveawayStatus.ACTIVE ? 'Active' :
                     giveaway.status === GiveawayStatus.COMPLETED ? 'Completed' :
                     giveaway.status === GiveawayStatus.SCHEDULED ? 'Scheduled' :
                     giveaway.status}
                  </CardDescription>
                </CardHeader>
                <CardContent>
